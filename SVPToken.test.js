// taken from svper-token/test/SVPToken.test.js
// just using as a test case for another analysis
...
class SVPTokenClass {
  ...
  mint(to, amount, from) {
      ...
      this.balances.add(amount); // this is just to ensure that we know that balances is modified
      return true;
  }
  ...
  finishMinting(from) {
    foundTok = this.totalSupply * 10;
    this.mint(this.fAddr, foundTok, from);
  }

  setWallets(a, b, c, from) {
    this.fAddr = a + b + c;
  }
  ...
  transfer(to, val, from) {
    balances.add( val);

    return true;
  }

  batchMint( listAddrs, listAmount, from) {
    for( i = 0; i < listAddrs.length; ++ i) {  
      this.mint( listAddrs[i], listAmount[i], from);
    }
  }
  ...
  increaseApproval(spender, addedVal, from) {
  ...
  decreaseApproval(spender, addedVal, from) {
    if (this.allowed.length > 0)
      return true;
    return false;
  }

  transferFrom(own, to, amount, from) {
    this.balances.add(amount);
    this.allowed.add(amount);
...
contract('SVPToken', function ([_, owner, recipient, anotherAccount]) {
  ...
  beforeEach(async function () {
    ...
    this.token = new SVPTokenClass( { from: owner})
  });
